...
import { compareLatestIssue, createIssueTargetUrl, type CompareViewport } from "./verify";
...
const fixLoopValueOptions = ["--to", "--target", "--tolerance", "--max-attempts", "--template", "--config"];

export async function runFixLoop(root: string, args: string[], config: FixLoopConfig): Promise<void> {
  ...
  if (agent !== "codex" && agent !== "claude") {
    throw new VernierError("VERNIER_INVALID_OPTION", "Usage: vernier fix-loop [all|<issue-id>] --to codex|claude", "Set agents.default in vernier.config.json or VERNIER_AGENT to avoid passing --to every time.");
  }

  const issues = await readFixLoopIssues(root, parsed.positionals()[0] ?? "all");
  ...
  if (parsed.flag("--print")) {
    console.log(renderFixLoopTask(issues, template, resolveTargetOption(args, config), maxAttempts, tolerance));
    return;
  }

  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    console.log(`[vernier] fix-loop attempt ${attempt}/${maxAttempts}`);
    const task = renderFixLoopTask(issues, template, resolveTargetOption(args, config), maxAttempts, tolerance);
    const result = await runAgent(agent, task);

    if (result === "missing") {
      console.log(`Could not find the ${agent} CLI on PATH.`);
      console.log("Run again with --print and paste the task into the desktop app, or install the CLI.");
      return;
    }
    ...
    for (const issue of issues) {
      ...
      const comparison = await compareLatestIssue(issue, targetUrl, tolerance, [capturedViewport(issue)]);
  ...
  console.log(`[vernier] fix-loop stopped with ${issues.length} todo issue${issues.length === 1 ? "" : "s"} remaining.`);
}

async function readFixLoopIssues(root: string, reference: string): Promise<IndexedVernierIssue[]> {
  if (reference === "all") {
...
function renderFixLoopTask(
  ...
  const issueTask = issues.length === 1 ? renderIssueTask(issues[0]!, template) : renderIssuesTask(issues, template);
  const verifyCommands = issues.map((issue) => `vernier verify ${issue.stableId} --compare --target ${target} --tolerance ${tolerance}`);
...
function readMaxAttempts(args: string[]): number {
  const value = parseArgs(args, { valueOptions: fixLoopValueOptions }).option("--max-attempts") ?? "1";
  const attempts = Number(value);

  if (!Number.isInteger(attempts) || attempts < 1 || attempts > 10) {
    throw new VernierError("VERNIER_INVALID_OPTION", `Invalid --max-attempts value: ${value}`, "Use an integer from 1 to 10.");
  }
...
function readTolerance(args: string[], config: FixLoopConfig): number {
  const value = parseArgs(args, { valueOptions: fixLoopValueOptions }).option("--tolerance") ?? String(config.verification?.bboxTolerancePx ?? 2);
...
function readAgentTemplate(args: string[], fallbackAgent: "codex" | "claude"): AgentTemplate {
  const value = parseArgs(args, { valueOptions: fixLoopValueOptions }).option("--template") ?? fallbackAgent;
