...
export async function importSessionArtifact(
  root: string,
  args: string[],
): Promise<string> {
  ...
  if (!source) {
    throw new VernierError(
      ...
      "Usage: vernier import <session-directory-or-zip>",
      "Pass a Vernier session directory or a zip created by `vernier export --format zip`.",
    );
  }

  const sourcePath = path.resolve(root, source);
  const feedbackDirectory = resolveFeedbackDirectory(
    root,
    parsed.option("--out-dir") ?? ".ui-feedback",
  );
  const importedDirectory = path.join(
    feedbackDirectory,
    "sessions",
    `imported-${new Date().toISOString().replace(/[:.]/g, "-")}`,
  );
...
async function validateImportedSession(
  ...
  try {
    ...
    throw new VernierError(
      "VERNIER_INVALID_IMPORT",
      "Imported artifact does not contain a readable session.json.",
      "Import a directory or zip created by Vernier.",
    );
  }
  ...
  if (!session.sessionId || !Array.isArray(session.issues)) {
    throw new VernierError(
      "VERNIER_INVALID_IMPORT",
      "Imported session.json is not a Vernier session.",
      "Check that the artifact contains Vernier session metadata.",
    );
  }
...
async function extractZip(zipPath: string, destination: string): Promise<void> {
  ...
  while (offset + 4 <= buffer.byteLength) {
    ...
    if (compression !== 0) {
      throw new VernierError(
        "VERNIER_INVALID_IMPORT",
        "Compressed zip entries are not supported.",
        "Use archives produced by `vernier export --format zip`.",
      );
    }
    ...
    if (compressedSize !== uncompressedSize || dataEnd > buffer.byteLength) {
      throw new VernierError(
        "VERNIER_INVALID_IMPORT",
        "Zip entry sizes are invalid.",
        "The import artifact may be corrupted.",
      );
    }
  ...
  if (extractedEntries === 0) {
    throw new VernierError(
      "VERNIER_INVALID_IMPORT",
      "Zip artifact did not contain any session files.",
      "Use an archive produced by `vernier export --format zip`.",
    );
  }
}
...
function safeDestinationPath(root: string, entryName: string): string {
  ...
  if (normalized.startsWith("/") || normalized.split("/").includes("..")) {
    throw new VernierError(
      ...
      "Zip entries must stay inside the Vernier session directory.",
    );
  }
