...
await writeFile(path.join(sessionDirectory, "session.json"), `${JSON.stringify(createLegacySession(), null, 2)}\n`);
...
assert(editedIssue.stableId === legacyIssue.stableId, "fallback stable ID should survive note edits");
...
function createLegacySession() {
  ...
  return {
    ...
    fullPageScreenshot: { ...screenshot, name: "full-page.png", kind: "full-page" },
    issues: [
      {
        id: 1,
        kind: "single",
